const listAccounts = [
  ...
  const listBalance = listAccounts.filter(value => value.balance >500000)
  ...
  console.log(`===========after editing property==========`)
  console.log(newListAccount);

  const totalBalance = listAccounts.reduce((total, value) => {
       return total +=value.balance;
  }, 0)

  console.log(`total balance: ${totalBalance}`);

  const statisticsByAge = listPersons.reduce( (statistics, value) =>{
      if(value.age < 18) statistics.child++;
      else if(value.age < 30) statistics.adult++;
